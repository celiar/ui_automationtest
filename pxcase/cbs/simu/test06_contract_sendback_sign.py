...
class ContractSendbackSign(TestBaseWeb):
    ...
    def test01_contract_sendback(self):
        ...
        try:
            ...
            # print(pname)
            # 应该与已签约订单的产品名称做比对
            if pname == ProductName.simu:
                print("【{}】分部寄回合同成功！".format(ProductName.simu))
            else:
                self.assertTrue(False, "【{}】分部寄回失败，请检查！".format(ProductName.simu))
        except:
            print("分部寄回合同执行失败，请检查！")
    ...
    def test02_contract_sign(self):
        ...
        try:
            ...
            # print(status)
            if status == "总部已签收":
                print("【{}】总部签收合同成功！".format(ProductName.simu))
            else:
                self.assertTrue(False, "【{}】总部签收失败，请检查！".format(ProductName.simu))
        except:
