...
class ProductOnline(TestBaseWeb):
    ...
    def test01_contract_distribute(self):
        ...
        try:
            ...
            sleep(1)
            l.findby("id", "productNameGive").send_keys(ProductName.simu)
            ...
            self.driver.get(cbshost+"/contractDistribute/search")
            sleep(1)
            l.findby("id", "productName").send_keys(ProductName.simu)
            sleep(1)
            ...
            sleep(1)
            if status == "总部已发放":
                print("【{}】总部分发合同成功！".format(ProductName.simu))
            else:
                print("合同分发失败，请检查！")
        except:
            print("合同发放执行失败，请检查！")
    ...
    def test02_contract_sign(self):
        ...
        try:
            ...
            # print(status)
            if status == "分部已签收":
                print("【{}】分部签收合同成功！".format(ProductName.simu))
            else:
                print("分部签收失败，请检查！")
        except:
            print("分部签收执行失败，请检查！")
    ...
    def test03_product_online(self):
        ...
        for i in range(10):
            ...
            # print(product_name)
            if product_name == ProductName.simu:
                ...
                if status1 == "预热中":
                    ...
                    print("【{}】已上线，状态为募集中！".format(ProductName.simu))
                    break
        sleep(1)
        l.findby("xpath", "//input[@name='name']").send_keys(ProductName.simu)
        sleep(1)
        l.findby("linktext", "查询").click()
        sleep(1)
        ...
        # print(status2)
        if status2 == "募集中":
            print("【{}】上线成功，状态为{}".format(ProductName.simu, status2))
        else:
            print("【{}】上线失败，请检查！".format(ProductName.simu))