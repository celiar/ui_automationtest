...
class AddProductSimu(TestBaseWeb):
    '''cbs产品管理-新增私募基金产品'''
    def test01_basic_info(self):
        ...
        l.send_keys('产品列表', '产品名称输入', ProductName.simu)
        l.click('产品列表', '查询')
        sleep(1)
        try:
            # 验证当日是否已创建产品
            ...
            self.assertIsNot(pname,ProductName.simu,'{}已存在，状态为{}'.format(ProductName,status))
        except:
            # 测试产品-非微动力-产品来源-管理人
            ...
            l.findby("linktext", "私募基金").click()
            sleep(1)
            l.findbyx("productTypeId").click()
            l.findby("linktext", "类固收类").click()
            sleep(1)
            l.findbyx("achievementType").click()
            l.findby("linktext", "创新产品").click()
            sleep(1)
            l.findbyx("raiseType").click()
            l.findby("linktext", "非定向募集").click()
            sleep(1)
            l.findbyx("attribute").click()
            l.findby("linktext", "资管计划").click()
            sleep(1)

            # 产品名称-币种-封闭期-认购起点-递增金额
            l.findby("id", "name").send_keys(ProductName.simu)
            sleep(1)
            ...
            l.findby("id", "investCategory").send_keys("投向一")
            sleep(1)
    ...
    def test03_account(self):
        ...
        if tag1=='募集账户户名':
            ...
            try:
                ...
                l.findsby("css", "button")[1].click()
                sleep(1)
                if ProductName.simu in l.findsby("xpath", "//td[@class='name']/a")[0].text:
                    print("新增产品--募集账号执行成功！")
                else:
                    self.assertTrue(False, "产品已存在，请检查")
            except:
    ...
    def test05_submit_audit(self):
        ...
        try:
            ...
            self.assertTrue(False,'依赖流程异常，产品列表不存在{}'.format(ProductName.simu))
        if pname == ProductName.simu and status1=="待审核":
    ...
    def test06_audit_pass(self):
        ...
        sleep(2)
        l.send_keys('产品审核', '产品名称输入', ProductName.simu)
        l.click('产品审核', '查询')
        try:
            pname = l.findsby("xpath", "//td[@class='red']")[0].text
            status1 = l.findsby("xpath", "//td[@class='red']/../td[8]")[0].text
        except:
            self.assertTrue(False,"产品审核列表不存在{}".format(ProductName.simu))

        if pname == ProductName.simu and status1 == "待上线":
            ...
            try:
                ...
                if status2 == "预热中":
                    print("【{}】审核通过，状态为预热中".format(ProductName.simu))
            except:
                print("【{}】审核未通过".format(ProductName.simu))
                self.assertTrue(False, "产品已存在，请检查！")

        elif pname == ProductName.simu and status1 == "预热中":
            print("【{}】已存在，状态为预热中".format(ProductName.simu))
        else:
            print("产品审核列表不存在{}".format(ProductName.simu))
